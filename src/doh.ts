...
import { isPublicIp, type Address } from './ip-policy.js'
import { withSignal } from './abort.js'

const MAX_DNS_BYTES = 65535
const MAX_CNAME_HOPS = 8
export interface DohResponse { body: Buffer; ageSeconds: number }
export type DohTransport = (url: URL, body: Buffer, signal: AbortSignal) => Promise<DohResponse>
export interface ValidationResult { addresses: Address[]; expiresAt: number }
const fail = (message: string): WebError => new WebError(`DoH: ${message}`, 'WEB_PROVIDER_ERROR')

export function canonicalName(input: string): string {
  const name = input.toLowerCase().replace(/\.$/, '')
  if (!name || name.length > 253 || name.split('.').some(label => !label || label.length > 63 || !/^[a-z0-9_-]+$/.test(label))) throw fail('invalid DNS name')
  return name
}

/** The operator-configured HTTPS endpoint is the trust anchor, not the target's OS DNS answer. */
export const requestDoh: DohTransport = (url, body, signal) => new Promise((resolve, reject) => {
  ...
  const request = httpsRequest(url, {
    ...
    if (response.statusCode !== 200 || response.headers['content-type']?.split(';')[0]?.trim().toLowerCase() !== 'application/dns-message') {
      reject(fail('expected HTTP 200 application/dns-message')); response.destroy(); return
    }
    const length = Number(response.headers['content-length'])
    if (length > MAX_DNS_BYTES) { reject(fail('response too large')); response.destroy(); return }
    const age = response.headers.age === undefined ? 0 : Number(response.headers.age)
    if (!Number.isSafeInteger(age) || age < 0) { reject(fail('invalid HTTP Age')); response.destroy(); return }
    const chunks: Buffer[] = []
    let bytes = 0
    response.on('data', (chunk: Buffer) => {
      bytes += chunk.length
      if (bytes > MAX_DNS_BYTES) { reject(fail('response too large')); response.destroy(); return }
      chunks.push(chunk)
    })
...
export class DohClient {
  constructor(
    ...
    private readonly transport: DohTransport = requestDoh,
    private readonly now: () => number = Date.now,
  ) {}

  async validate(hostname: string, signal: AbortSignal): Promise<ValidationResult> {
    ...
    try {
      ...
      if (results.every(result => result.terminalCount === 0)) throw fail('no A or AAAA addresses')
  ...
  private async query(name: string, type: 'A' | 'AAAA', signal: AbortSignal): Promise<{ message: DecodedPacket; receivedAt: number; age: number }> {
    signal.throwIfAborted()
    ...
    try {
      ...
      if (!response.body.length || response.body.length > MAX_DNS_BYTES || !Number.isSafeInteger(response.ageSeconds) || response.ageSeconds < 0) throw fail('invalid response size or age')
      let message: DecodedPacket
      try {
        message = packet.decode(response.body)
        if (packet.decode.bytes !== response.body.length) throw new Error('trailing bytes')
      } catch { throw fail('malformed DNS message') }
      ...
      if (message.additionals?.some(answer => answer.type === 'OPT' && (answer.extendedRcode !== 0 || answer.ednsVersion !== 0))) throw fail('unsupported or failed EDNS response')
      return { message, receivedAt: this.now(), age: response.ageSeconds }
    } finally { clearTimeout(timer) }
  }

  private async resolveType(start: string, type: 'A' | 'AAAA', signal: AbortSignal): Promise<ValidationResult & { terminalCount: number }> {
    let name = start
    let hops = 0
    ...
    for (;;) {
      ...
      const ttlExpiry = (ttl: number | undefined): number => receivedAt + Math.max(0, Number.isInteger(ttl) && ttl! >= 0 ? ttl! - age : 0) * 1000
      ...
      for (const answer of [...(message.answers ?? []), ...(message.additionals ?? []), ...(message.authorities ?? [])]) {
        ...
        if (isIP(answer.data) !== family) throw fail('invalid address record')
        if (!isPublicIp(answer.data)) throw new WebError('DoH returned a non-public destination', 'WEB_BLOCKED_URL')
      ...
      for (;;) {
        ...
        if (records.some(answer => !('class' in answer) || answer.class !== 'IN')) throw fail('unsupported DNS class')
        const addresses = records.filter(answer => answer.type === type)
        const aliases = records.filter((answer): answer is Answer & { type: 'CNAME'; data: string } => answer.type === 'CNAME')
        if (aliases.length > 1 || (aliases.length && records.some(answer => answer.type === 'A' || answer.type === 'AAAA'))) throw fail('conflicting alias records')
        if (addresses.length) return { addresses: observed, terminalCount: addresses.length, expiresAt }
        if (aliases.length) {
          const alias = aliases[0]!
          expiresAt = Math.min(expiresAt, ttlExpiry(alias.ttl))
          name = canonicalName(alias.data)
          if (++hops > MAX_CNAME_HOPS || seen.has(name)) throw fail('CNAME loop or hop limit exceeded')
          seen.add(name)
          followed = true
          continue
        }
        ...
        if (soa.length) {
          for (const answer of soa) {
            if (answer.type === 'SOA') expiresAt = Math.min(expiresAt, ttlExpiry(Math.min(answer.ttl ?? 0, answer.data.minimum ?? 0)))
          }
          return { addresses: observed, terminalCount: 0, expiresAt }
        }
        if (followed) break // The recursive server omitted the terminal data: query the alias explicitly.
        if (message.answers?.length) throw fail('incomplete or unrelated DNS answer')
