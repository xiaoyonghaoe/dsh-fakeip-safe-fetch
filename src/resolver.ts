...
export class SafeResolver {
  ...
  readonly resolve: HttpFetchResolver = async (hostname, callerSignal) => {
    ...
    if (literal) throw new WebError(`IP literal ${rawHost} is never treated as a fake-IP destination`, 'WEB_BLOCKED_URL')
    // Report the offending addresses: an answer that Clash synthesises for both families fails
    // as soon as one family is missing from fakeIpCidrs, and the bare verdict is hard to act on.
    const foreign = entries.filter(entry => !isPublicIp(entry.address) && !isFakeIp(entry.address, this.cidrs))
    const publicOnly = entries.filter(entry => isPublicIp(entry.address))
    if (foreign.length || publicOnly.length) {
      const reasons = [
        publicOnly.length ? `public ${publicOnly.map(entry => entry.address).join(', ')} mixed with fake-IP` : '',
        foreign.length ? `non-public ${foreign.map(entry => entry.address).join(', ')} outside the configured fakeIpCidrs; add your proxy's fake-IP ranges` : '',
      ].filter(Boolean)
      throw new WebError(`DNS answer is not entirely fake-IP: ${reasons.join('; ')}`, 'WEB_BLOCKED_URL')
    }
